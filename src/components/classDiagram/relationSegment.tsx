...
function RelationSegment(props: { graphicData: ISegmentGraphicDat, functionality: IElementFunctionality}) {
    return (
        <path onClick={(ev) => props.functionality.onSegmentMove(ev, props.graphicData.id)} cursor={props.graphicData.direction === Direction.HORIZONTAL ? 'ns-resize' : 'ew-resize'} stroke='black' d={`M ${props.graphicData.x} ${props.graphicData.y} l ${props.graphicData.lineToX} ${props.graphicData.lineToY}`}/>
    )
}
